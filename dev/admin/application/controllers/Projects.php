...
class projects extends CI_Controller
{
	...
	public function index(){
	     ...
	     $data['title']='projects Page view';
	     $data['menu']='projects';
	     $data['child_menu']='';
	     $data['projects_pages']=$this->app_model->getAllList('cms_pages',array('type' =>'post' ),'id','DESC');
	     $this->load->view('admin/projects/manage-projects',$data);
 ...
 if(!empty($_FILES['files']['name'][0])){
         ...
         $data=array(
        	...
        	if ($q==true) {
        	    ...
        	    $this->session->set_flashdata('message', 'projects Page Add Successfully.');
        	    redirect('projects/add');
        		
        	} else {
        		$this->session->set_flashdata('class', 'danger');
        		$this->session->set_flashdata('message', 'projects Page Add Failed.');
        		redirect('add');
        }
    ...
    public function edit(){
		...
		$data['id'] = $id;
		$data['child_menu']='';
		$data['projects']= $this->app_model->get_special_details('cms_pages',array('id'=>$id,'type'=>'post'));
 ...
 if(!empty($_FILES['files']['name'][0])){
	    	...
	    	if ($q==true) {
	    		$this->session->set_flashdata('class', 'success');
	    	    $this->session->set_flashdata('message', 'projects Update Successfully.');
	    	    redirect('projects/edit/'.$id);
	    		
	    	} else {
	    		$this->session->set_flashdata('class', 'danger');
	    		$this->session->set_flashdata('message', 'projects Update Failed.');
	    		redirect('projects/edit/'.$id);
	 ...
	 public function status($status,$id){
   		...
   		if ($status==1) {
   			...
   			$message='Active projects Successfully.';
   			$class='success';
   		} else {
   			$q = $this->app_model->update('cms_pages',array('status'=>$status),array('id'=>$id));
   			$message='Block projects Successfully.';
   			$class='error';
   		}
   		if ($q==true) {
				...
				$this->session->set_flashdata('message', 'projects Status Change Failed.');
				redirect('projects');
		    }
     }
